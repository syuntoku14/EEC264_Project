...
r = 1.0;
r_specific_simulation(r, r_max, 1);

r = 3.0;
r_specific_simulation(r, r_max, 2);

rmpath('../utils');
rmpath('../filter');
rmpath('../model');

function r_specific_simulation(r, r_max, n_figure)
    %% Simulation
    specific_err_list = sim_classicKF_specific_r(r, r);
    mm_err_list = sim_classicKF_specific_r(r, r_max);
    ibr_err_list = sim_IBRKF(r);

    figure(n_figure); hold on;
    plot(specific_err_list);
    plot(mm_err_list, 'LineStyle', '-.');
    plot(ibr_err_list);
    legend('Model-Specific', 'Minimax', 'IBR'); %, 'IBR Kalman');
    xlabel('k'); ylabel('MSE');
    hold off;
end